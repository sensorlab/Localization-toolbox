"""
    ...
    ********************
    ...
    Version 0.0: Tomaz Javornik (January 2017)
...
Agents_Size = 10                            # number of Agents/refAgents in network
delta_RSSI = 1.0                            # precision of RSSI measurement  in dBm
...
IJS_Outdoor.append("Raster Map", "RSSI maps", RSSI_maps)

# Extract measurements form the map
...
RSSI_trace.add_error("Rayleigh")           # add Rayleigh fadding
RSSI_trace.quantize_vals(2.0)               # add Measurement quantization
...
if plot_net:
    ...
    fig, ax  = Region.plot_Network(2, False, 'oc', 10, Anchors, False, "Anchors", reg)
    fig, ax = Region.plot_Network(2, False, 'or', 7, Agents, False, "Loc-Est", reg)
    fig, ax = Region.plot_Network(2, True, 'sk', 5, refAgents, False, "Loc-Real", reg)
    ax.grid()
    ax.set_aspect('equal')
